...
def page_count(path: Path) -> int:
    with path.open('rb') as stream:
        ...
        if reader.is_encrypted:
            raise ConversionError('ENCRYPTED PDF CANNOT BE PRINTED AUTOMATICALLY')
        count = len(reader.pages)
        if not count:
            raise ConversionError('NO PDF PAGES FOUND')
        return count


def safe_environment():
    return {k: v for k, v in os.environ.items() if k in {'PATH', 'HOME', 'USER', 'LOGNAME', 'LANG', 'TMPDIR', 'XDG_RUNTIME_DIR'}} | {'LC_ALL': 'C', 'SAL_USE_VCLPLUGIN': 'gen'}
...
class LibreOfficeRenderer:
    ...
    def render(self, group, directory):
        ...
        for index in range(len(group.headers)):
            width = min(32, max(10, max(len(str(row[index] or '')) for row in all_rows) + 2))
            widths.append(width)
        ...
        for row_index, row in enumerate(all_rows, 1):
            ...
            for column, value in enumerate(row, 1):
                ...
                if isinstance(value, str):
                    cell.data_type = 's'  # Literal strings beginning '=' must never become formulas.
                ...
                lines = max(lines, sum(max(1, math.ceil(len(part) / max(1, widths[column - 1] - 2))) for part in str(value or '').split('\n')))
        ...
        sheet.page_setup.fitToHeight = 0  # Do not shrink arbitrarily long reports into unreadable one-page output.
...
<item oor:path="/org.openoffice.Office.Common/Security/Scripting"><prop oor:name="MacroSecurityLevel" oor:op="fuse"><value>3</value></prop></item>
            ...
            try:
                result = subprocess.run(['libreoffice', f'-env:UserInstallation={profile.as_uri()}', '--headless', '--nologo', '--nodefault', '--nofirststartwizard', '--norestore', '--convert-to', 'pdf:calc_pdf_Export', '--outdir', str(directory), str(workbook_path)],
                                        env=safe_environment(), capture_output=True, text=True, timeout=self.timeout, check=False)
            ...
            if result.returncode or not pdf_path.is_file():
                raise ConversionError('EXCEL CONVERSION FAILED: LibreOffice produced no usable PDF')
