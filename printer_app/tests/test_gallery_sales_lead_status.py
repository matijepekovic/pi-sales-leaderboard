...
def _seed(repository, *, day=DAY, ident='card', work_order='00000007'):
...
def _record(number='00000007', lead='lead-1', status='Sold'):
    return dict(work_order_number=number, lead_source_id=lead, sales_lead_status=status)


def _publish(repository, status='Sold', *, number='00000007', lead='lead-1', captured=100):
...
def test_existing_displayed_status_survives_until_successful_direct_lookup(repository):
    ...
    assert repository.replace_work_order_lead_statuses(['00000007'], [], 201) == 1
...
def test_status_propagates_across_all_dates_and_orders_of_same_lead_only(repository):
    for ident, number, day in [('old', '00000008', '2020-01-01'), ('future', '00000007', '2030-01-01'),
                               ('undated', '00000008', None), ('different-lead', '00000009', DAY),
                               ('unlinked', '00000010', DAY)]:
        _seed(repository, ident=ident, work_order=number, day=day)
    with repository.connect() as c:
        c.execute("UPDATE items SET state='REVIEW' WHERE id='undated'")
    repository.replace_work_order_lead_statuses(['00000007', '00000008', '00000009'], [
        _record(status='Open'), _record('00000008', status='Open'), _record('00000009', 'lead-2', 'New'),
    ], 100)
...
def test_completed_lookup_clears_unknown_order_mapping_but_not_other_orders(repository, replacement):
    _seed(repository, ident='other-order', work_order='00000008')
    repository.replace_work_order_lead_statuses(['00000007', '00000008'], [_record(), _record('00000008')], 100)
    rows = [] if replacement == 'missing' else [_record(lead='')] if replacement == 'blank-id' else [
        _record(), _record(lead='another-lead')]
    assert repository.replace_work_order_lead_statuses(['00000007'], rows, 200) == 1
    ...
    assert repository.replace_work_order_lead_statuses(['00000007'], rows, 201) == 0
...
def test_older_results_cannot_restore_missing_mapping_or_overwrite_current_status(repository):
    ...
    assert repository.item('card')['sales_lead_status'] == 'Sold'
    repository.replace_work_order_lead_statuses(['00000007'], [], 300)
...
def test_conflicting_status_at_same_capture_stays_unavailable_until_newer_result(repository, single_batch):
    if single_batch:
        repository.replace_work_order_lead_statuses(['00000007'], [_record(status='Open'), _record()], 100)
    else:
        _publish(repository, 'Open', captured=100)
...
def test_unrequested_records_cannot_change_cached_associations(repository):
    _publish(repository)
    repository.replace_work_order_lead_statuses(['00000008'], [_record(lead='wrong-lead', status='New')], 200)
    assert repository.item('card')['sales_lead_status'] == 'Sold'
    assert repository.item('card')['lead_source_id'] == 'lead-1'


def test_late_card_uses_cached_mapping_and_latest_shared_status_without_appointment(repository):
    _publish(repository, 'Open', number='00000008')
    _publish(repository, captured=200)
    _seed(repository, ident='late', work_order='00000008', day=None)
...
@pytest.mark.parametrize('work_order,expected_lead,expected_status', [
    ('00000007', 'lead-1', 'Sold'), ('00000008', 'lead-2', 'New'), ('00000009', '', ''),
])
def test_corrected_work_order_reattaches_its_own_cached_lead(repository, work_order, expected_lead, expected_status):
    _publish(repository)
    _publish(repository, 'New', number='00000008', lead='lead-2')
    repository.correct_lead('card', 'Jordan Example')
    before = repository.item('card')
    repository.repair_recognition('card', before['text'].replace('00000007', work_order),
...
def test_delayed_refresh_guard_ignores_old_work_order_but_never_requires_date(repository):
    ...
    assert repository.refresh_sales_lead_status('card', expected_work_order_key='00000008') == 0
    assert repository.item('card')['sales_lead_status'] == 'Open'
    repository.correct_date('card', None)
    assert repository.refresh_sales_lead_status('card', expected_work_order_key='00000007') == 1
    assert repository.item('card')['sales_lead_status'] == 'Sold'
...
def test_appointment_snapshots_and_enrichment_cannot_change_shared_lead_status(repository):
    ...
    repository.apply_reference('card', 'appointment', 'final', 'Example Rep', row['text'],
                               row['lead_name'], row['address'], expected_day=DAY, expected_work_order_key='00000007')
    assert repository.item('card')['sales_lead_status'] == 'Sold'
    assert repository.item('card')['lead_source_id'] == 'lead-1'
    assert repository.sales_status_for_lead('wrong-lead') == ''
    assert repository.reference_matches(DAY, 'final', '00000007')[0]['sales_lead_status'] == ''


def test_requested_work_orders_include_all_retained_cards_but_not_expired_reference_history(repository):
    _seed(repository, ident='undated', day=None, work_order='00000008')
    _seed(repository, ident='future', day='2030-01-01', work_order='00000009')
    _seed(repository, ident='duplicate', day='2020-01-01', work_order='00000007')
    _seed(repository, ident='deleted', work_order='00000010')
    with repository.connect() as c:
        c.execute("UPDATE items SET state='REVIEW' WHERE id='undated'")
        c.execute("UPDATE items SET state='DELETING' WHERE id='deleted'")
    repository.replace_reference_snapshot('2020-01-01', 'morning', [dict(source_id='ref', work_order_number='00000011')], 100)
    assert repository.work_order_numbers() == ['00000007', '00000008', '00000009']


def test_housekeeping_keeps_reference_only_mapping_for_later_scan_and_removes_orphans(repository):
    _publish(repository)
    _publish(repository, number='00000008')
    repository.replace_reference_snapshot(DAY, 'morning', [dict(source_id='ref', work_order_number='00000008')], 100)
    repository.housekeeping(0)
    _seed(repository, ident='late', work_order='00000008')
    assert repository.item('late')['sales_lead_status'] == 'Sold'
