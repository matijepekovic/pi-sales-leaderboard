...
(function(){
  ...
  function dataFilters(report){
    ...
    return `<div class="subcard" style="margin-top:14px"><div class="toolbar"><div><strong>Data Filters</strong><div class="small">These change what the Source pulls. They are separate from Display Filters.</div></div><button class="btn" data-action="add-data-filter" ${state.columns?"":"disabled"}>+ Data Filter</button></div>${filters.length?filters.map((filter,index)=>`<div class="rule-row" style="grid-template-columns:1fr 1fr auto;margin-top:9px"><div><label>Source field</label><select data-data-filter-field="${index}">${filterFieldOptions(filters,index)}</select></div><div><label>Value</label>${filterValueControl(filters,index)}</div><button class="btn danger" data-action="remove-data-filter" data-index="${index}">Remove</button></div>`).join(""):'<div class="small" style="margin-top:9px">No Data Filters.</div>'}</div>`;
  }
  ...
  function render(){
    ...
    host.innerHTML=`<div class="card"><div class="toolbar"><div><h2>Sources</h2><div class="small">Where Stats gets data.</div></div><button class="btn primary" data-action="new-source">+ Source</button></div><div class="stack" style="margin-top:12px">${state.sources.map(sourceCard).join("")||'<div class="small">No Sources configured.</div>'}</div></div>${sourceEditor()}<div class="card"><div class="toolbar"><div><h2>Reports</h2><div class="small">Pulled datasets available to Filters and Screens.</div></div><button class="btn primary" data-action="new-report" ${state.sources.length?"":"disabled"}>+ Report</button></div><div class="stack" style="margin-top:12px">${state.reports.map(reportCard).join("")||'<div class="small">No Reports yet.</div>'}</div></div>${reportEditor()}${inspection()}<div class="status">${esc(state.message||"")}</div>`;
    bind();
  }
