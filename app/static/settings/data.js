...
(function(){
  ...
  const state={
    ...
    columns:null,message:"",reportSearch:""
  };
  let loaded=false;
  ...
  function reportBrowser(){
    ...
    for(const report of unmatched){
      add("Other",report.name,configuredReportCard(report));
    }

    if(!groups.size)return '<div class="small">No Report values returned by this Source.</div>';

    const sorted=Array.from(groups.entries()).sort((a,b)=>a[0].localeCompare(b[0],undefined,{sensitivity:"base"}));
    return `<div><input id="reportSearch" type="search" value="${esc(state.reportSearch)}" placeholder="Search reports…" aria-label="Search reports" style="margin-bottom:12px"><div id="reportGroups" class="stack">${sorted.map(([group,items])=>`<details class="subcard" data-report-group open><summary style="cursor:pointer;font-weight:900;font-size:16px">${esc(group)} <span class="small">(${items.length})</span></summary><div class="stack" style="margin-top:10px">${items.sort((a,b)=>a.label.localeCompare(b.label,undefined,{sensitivity:"base"})).map(item=>`<div data-report-item data-search="${esc(`${group} ${item.label}`.toLowerCase())}">${item.html}</div>`).join("")}</div></details>`).join("")}</div></div>`;
  }
  ...
  function render(){
    ...
    if(!host)return;
    const source=selectedSource();
    host.innerHTML=`${sourceEditor()}<div class="card"><div class="toolbar"><div><h2>Reports</h2><div class="small">Reports available from ${esc(source?.name||"the selected Source")}.</div></div>${source?'<button class="btn" data-action="reload-values">Reload</button>':""}</div><div style="margin-top:12px">${reportBrowser()}</div></div>${reportEditor()}${inspection()}<div class="status">${esc(state.message||"")}</div>`;
    bind();
    renderSourceMenu();
    applyReportSearch();
  }
  ...
  function syncReport(){
    ...
    report.source_value=hostValue('[data-report="source_value"]')||report.source_value;
    rt.date_mode=hostValue('[data-report="date_mode"]')||'current_month';
    rt.date_start=hostValue('[data-report="date_start"]');
    rt.date_end=hostValue('[data-report="date_end"]');
  ...
  async function loadColumnsForEditor(quiet=false){
    ...
    try{
      state.columns=await runtime.api(`/api/data/reports/${encodeURIComponent(report.id)}/columns`,runtime.json("POST",{}));
      if(!quiet)state.message="";
  ...
  async function saveReport(){
    ...
    try{
      ...
      let refreshError="";
      try{await runtime.api(`/api/data/reports/${encodeURIComponent(savedId)}/refresh`,{method:"POST"});}catch(error){refreshError=error.message;}
      const reports=await runtime.api("/api/data/reports");
      state.reports=reports.reports||[];
      const saved=reportBy(savedId);
      state.editor=saved?{type:"report",value:JSON.parse(JSON.stringify(saved))}:null;
      state.message=refreshError?`Report saved. Refresh failed: ${refreshError}`:"Report saved and refreshed.";
  ...
  function bind(){
    ...
    host.querySelectorAll("[data-action]").forEach(button=>button.addEventListener("click",async()=>{
      ...
      if(a==="close-inspection"){state.inspection=null;render();return;}
