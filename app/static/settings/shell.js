...
(function(){
  ...
  async function authStatus(){
    try{
      const data=await runtime.api("/api/auth/status");
  ...
  async function unlock(){
    const status=$("unlockStatus"),button=$("unlockBtn"),pin=$("unlockPin");
    if(!pin||!button)return;
    button.disabled=true;if(status)status.textContent="Unlocking…";
    try{
      await runtime.api("/api/auth/unlock",runtime.json("POST",{pin:pin.value}));
      pin.value="";if(status)status.textContent="";setLocked(false);
      runtime.emit("unlocked");
      runtime.emit("section",activeSectionId());
    }catch(error){if(status)status.textContent=error.message||"Could not unlock.";}
    finally{button.disabled=false;}
  }

  function activeSectionId(){
    return sections().find(section=>section.classList.contains("active"))?.id||"settingsData";
  }
  ...
  function activate(id,focus=false){
    ...
    $("settingsPageTitle").textContent=target.dataset.settingsTitle||"Settings";
    $("settingsPageDescription").textContent=target.dataset.settingsDescription||"";
    if($("settingsContent"))$("settingsContent").scrollTop=0;
    try{localStorage.setItem(ACTIVE_KEY,target.id);}catch(_){ }
    runtime.emit("section",target.id);
  }

  function navKeydown(event){
    const list=buttons(),index=list.indexOf(document.activeElement);if(index<0)return;
    let next=index;
  ...
  function boot(){
    ...
    $("settingsNav")?.addEventListener("keydown",navKeydown);
    $("settingsBack")?.addEventListener("click",()=>window.location.assign("/"));
    $("unlockBtn")?.addEventListener("click",unlock);
    $("unlockPin")?.addEventListener("keydown",event=>{if(event.key==="Enter")unlock();});
    document.addEventListener("stats:settings-locked",()=>setLocked(true));
    let wanted="settingsData";try{wanted=localStorage.getItem(ACTIVE_KEY)||wanted;}catch(_){ }
    activate(wanted);
    authStatus().then(unlocked=>{if(unlocked)runtime.emit("section",activeSectionId());});
  }
