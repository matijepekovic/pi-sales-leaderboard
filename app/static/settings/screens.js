...
(function(){
  ...
  const S={screens:[],reports:[],filters:[],editor:null,step:0,preview:null,message:""};
  ...
  async function load(){
    const [screens,reports,filters]=await Promise.all([R.api("/api/screens"),R.api("/api/data/reports"),R.api("/api/filters")]);
    S.screens=screens.screens||[];S.reports=reports.reports||[];S.filters=filters.filters||[];loaded=true;render();
  }
  ...
  function listHtml(){
    return `<div class="card"><div class="toolbar"><div><h2>Screens</h2><div class="small">Screens choose pulled Reports, reusable Filters, presentation and Theme.</div></div><button class="btn primary" data-action="new">+ Screen</button></div><div class="stack" style="margin-top:12px">${S.screens.map(x=>`<div class="subcard"><div class="toolbar"><div><strong>${esc(x.name)}</strong><div class="small">${(x.reports||[]).length} Reports · ${(x.filter_ids||[]).length} Filters · ${x.theme_mode==="custom"?"Custom Theme":"Inherited Theme"}</div></div><div class="row"><button class="btn" data-action="edit" data-id="${esc(x.id)}">Edit</button><button class="btn" data-action="open-preview" data-id="${esc(x.id)}">Display Preview</button><button class="btn danger" data-action="delete" data-id="${esc(x.id)}">Delete</button></div></div></div>`).join("")||'<div class="small">No Screens yet.</div>'}</div></div>`;
  }
  ...
  function filtersStep(v){return `<div><div class="toolbar"><div><h3>Assign Filters</h3><div class="small">Select the reusable Filters this Screen applies.</div></div><button class="btn" data-action="create-filter">+ Create Filter</button></div><div class="stack" style="margin-top:10px">${S.filters.map(f=>`<label class="choice"><input type="checkbox" data-filter="${esc(f.id)}" ${(v.filter_ids||[]).includes(f.id)?"checked":""}><span><strong>${esc(f.name)}</strong>${(f.rules||[]).map(rule=>`<br><span class="small">${esc(report(rule.report_id)?.name||rule.report_id)} · ${esc(rule.field)} ${esc(rule.operator)} ${esc(rule.value)}</span>`).join("")}</span></label>`).join("")||'<div class="small">No Filters yet. Create one here.</div>'}</div></div>`;}
  ...
  function displayStep(v){normalizeTables(v);return `<div><div class="toolbar"><div><h3>Create Display</h3><div class="small">Choose columns, ranking and row count. Live Preview uses the real pulled data and selected Filters.</div></div><button class="btn" data-action="refresh-preview">Refresh Preview</button></div><div class="stack" style="margin-top:10px">${(v.tables||[]).map(table).join("")||'<div class="small">Choose Reports first.</div>'}</div></div>`;}
  ...
  async function open(id=null){if(!loaded)await load();S.editor=id?JSON.parse(JSON.stringify(screen(id))):blank();S.step=0;S.preview=null;S.message="";normalizeTables(S.editor);render();schedule();}
