...
def blueprint(service):
    ...
    def themes_state():
        ...
        return jsonify({
            ...
            "themes": service.display_state(settings),
            "teams": service.public_teams(),
        })
    ...
    def add_library_item(asset_key):
        try:
            ...
            item_id, url = service.add_library_item(
                asset_key, upload, request.form.get("label") or getattr(upload, "filename", "")
            )
    ...
    def upload_theme_asset(scope, asset_key):
        try:
            ...
            version, theme = service.apply_asset(
                scope, asset_key, upload=upload, library_id=library_id
            )
            return jsonify({"ok": True, "settings_version": version, "theme": theme})
        except Exception as exc:
            return jsonify({"ok": False, "error": str(exc)}), 400
    ...
    def theme_asset(scope, asset_key):
        try:
            path = service.asset_path(scope, asset_key)
        except Exception:
            path = None
