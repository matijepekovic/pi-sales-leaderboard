...
def blueprint(service):
    ...
    @bp.get("/api/themes")
    def themes_state():
        settings = service.repos.settings.get()
        return jsonify({"ok": True, "manifest": service.manifest(), "themes": service.display_state(settings), "teams": service.public_teams()})

    @bp.put("/api/themes/<scope>")
    def save_theme(scope):
        try:
            version, theme = service.save_theme(scope, request.get_json(force=True) or {})
    ...
    @bp.delete("/api/themes/<scope>")
    def reset_theme(scope):
        try:
            version, theme = service.reset_theme(scope)
            return jsonify({"ok": True, "settings_version": version, "theme": theme})
        except Exception as exc: return jsonify({"ok": False, "error": str(exc)}), 400

    @bp.get("/api/screen-themes/<screen_id>")
    def screen_theme(screen_id):
        try: return jsonify({"ok": True, "manifest": service.manifest(), "theme": service.effective_screen_theme(screen_id)})
        except Exception as exc: return jsonify({"ok": False, "error": str(exc)}), 400
    ...
    def asset_library(): return jsonify({"ok": True, "items": service.library_state()})

    @bp.post("/api/asset-library/<asset_key>")
    def add_library_item(asset_key):
        try:
            upload = request.files.get("asset")
            item_id, url = service.add_library_item(asset_key, upload, request.form.get("label") or getattr(upload, "filename", ""))
    ...
    def library_item(asset_key, item_id):
        try: path = service.library_item_path(asset_key, item_id)
    ...
    def delete_library_item(asset_key, item_id):
        try: service.delete_library_item(asset_key, item_id); return jsonify({"ok": True})
        except Exception as exc: return jsonify({"ok": False, "error": str(exc)}), 400

    @bp.post("/api/themes/<scope>/assets/<asset_key>")
    def upload_theme_asset(scope, asset_key):
        try:
            ...
            version, theme = service.apply_asset(scope, asset_key, upload=upload, library_id=library_id)
            return jsonify({"ok": True, "settings_version": version, "theme": theme})
        except Exception as exc: return jsonify({"ok": False, "error": str(exc)}), 400

    @bp.delete("/api/themes/<scope>/assets/<asset_key>")
    def reset_theme_asset(scope, asset_key):
        try:
            version, theme = service.reset_asset(scope, asset_key)
            return jsonify({"ok": True, "settings_version": version, "theme": theme})
        except Exception as exc: return jsonify({"ok": False, "error": str(exc)}), 400
    ...
    def upload_screen_theme_asset(screen_id, asset_key):
        try:
            ...
            library_id = (body or {}).get("library_id") if isinstance(body, dict) else None
            upload = request.files.get("asset") if request.files else None
            version, theme = service.apply_screen_asset(screen_id, asset_key, upload=upload, library_id=library_id)
            return jsonify({"ok": True, "settings_version": version, "theme": theme})
        except Exception as exc: return jsonify({"ok": False, "error": str(exc)}), 400
    ...
    @bp.get("/api/theme-assets/<scope>/<asset_key>")
    def theme_asset(scope, asset_key):
        try: path = service.asset_path(scope, asset_key)
        except Exception: path = None
    ...
    def screen_theme_asset(screen_id, asset_key):
        try: path = service.screen_asset_path(screen_id, asset_key)
        except Exception: path = None
        if not path or not path.exists(): abort(404)
        return send_file(path, conditional=True)
