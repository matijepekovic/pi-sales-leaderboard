...
class ScreenService:
    ...
    def save(self, incoming):
        ...
        screen_id = str(incoming.get("id") or "").strip() or f"screen-{uuid.uuid4().hex[:12]}"
    ...
    def _filters_for(self, screen, report_id, field_keys):
        ...
        for item in screen.get("filters") or []:
            if str(item.get("field")) not in field_keys:
                continue
            scope = str(item.get("scope") or "screen")
            if scope == "report" and str(item.get("report_id") or "") != report_id:
                continue
            result.append(item)
        return result
