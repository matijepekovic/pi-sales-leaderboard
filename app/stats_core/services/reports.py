...
class ReportService:
    ...
    def _catalog(self): return self.repos.data_catalog.get()

    def _source(self, source_id):
        source = self.repos.data_catalog.source(source_id)
        if not source: raise ValidationError("Source not found.")
        return source

    def _adapter(self, source):
        key = str(source.get("adapter") or "")
        adapter = self.adapters.get(key)
        if not adapter: raise ValidationError(f"Source adapter '{key}' is not available.")
        return adapter
    ...
    def get(self, report_id):
        ...
        if not report: raise ValidationError("Report not found.")
        return report

    def list(self, source_id=None):
        rows = []
        for report in self._catalog()["reports"]:
            if source_id and str(report.get("source_id")) != str(source_id): continue
            item = dict(report)
    ...
    def fields(self, report_id):
        report = self.get(report_id); kind = str(report.get("kind") or "table")
        if kind == "rep_performance": return [{"key": k, "label": l, "type": t} for k,l,t in METRIC_DEFS]
        if kind == "product_close": return list(_PRODUCT_FIELDS)
    ...
    def status(self, report_id):
        report = self.get(report_id); kind = str(report.get("kind") or "table")
        if kind == "rep_performance": return {"status": self.repos.meta.get("source_status", ""), "last_refresh": self.repos.meta.get("last_source_refresh", "")}
    ...
    def refresh(self, report_id):
        report = self.get(report_id); source = self._source(report.get("source_id"))
        adapter, app_settings = self._adapter_settings(source); kind = str(report.get("kind") or "table")
        if kind == "rep_performance": return self.rep_refresh.refresh(adapter, app_settings, source, report)
        if kind == "product_close":
        ...
        saved = self.repos.report_data.replace(report_id, table.get("fields") or [], table.get("rows") or [], {
            "status": f"{len(table.get('rows') or [])} rows", "last_refresh": stamp,
    ...
    def save(self, incoming):
        ...
        source_id = str(incoming.get("source_id") or "").strip(); self._source(source_id)
        ...
        if report_id in (REP_REPORT_ID, PRODUCT_REPORT_ID): kind = "rep_performance" if report_id == REP_REPORT_ID else "product_close"
        elif kind != "table": kind = "table"
        ...
        report = {"id": report_id, "source_id": source_id, "name": name, "kind": kind, "source_config": dict(source_config or {}), "runtime": dict(runtime or {})}
        catalog = self._catalog(); catalog["reports"] = [r for r in catalog["reports"] if str(r.get("id")) != report_id] + [report]
        self.repos.data_catalog.save(catalog); self.repos.meta.bump("settings_version")
        return report

    def delete(self, report_id):
        report_id = str(report_id or "").strip()
        if report_id in (REP_REPORT_ID, PRODUCT_REPORT_ID): raise ValidationError("Built-in reports cannot be deleted.")
        catalog = self._catalog(); before = len(catalog["reports"])
        catalog["reports"] = [r for r in catalog["reports"] if str(r.get("id")) != report_id]
        if len(catalog["reports"]) == before: raise ValidationError("Report not found.")
        self.repos.data_catalog.save(catalog); self.repos.report_data.delete(report_id); self.repos.meta.bump("settings_version")
        return True
