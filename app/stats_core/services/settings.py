...
Routes no longer mutate the settings dictionary directly. This service owns
normalization, secret handling, feature access metadata and settings-version
bookkeeping.
"""
...
def clean_source(raw):
    ...
    if isinstance(raw.get("mapping"), dict):
        ...
        clean["mapping"] = {
            ...
            "metrics": {
                str(key): str(value)[:200] for key, value in metrics.items()
...
class SettingsService:
    ...
    def update(self, incoming):
        ...
        if active and ((parsed in CORE_MODES and can_use(parsed)) or (active == PRODUCT_MODE and can_use("product_close"))):
            current["active_mode"] = active
            if parsed == "per_team" and team:
                current["per_team_selected"] = team
        if isinstance(incoming.get("title"), str): current["title"] = incoming["title"][:80]
        if isinstance(incoming.get("subtitle"), str): current["subtitle"] = incoming["subtitle"][:120]
        ...
        for key in ("tableau_site", "tableau_pat_name", "tableau_view"):
            if isinstance(incoming.get(key), str): current[key] = incoming[key].strip()[:200]
        if isinstance(incoming.get("tableau_pat_secret"), str):
            value = incoming["tableau_pat_secret"].strip()
            if value: current["tableau_pat_secret"] = value
        if incoming.get("tableau_pat_clear") is True: current["tableau_pat_secret"] = ""
        if isinstance(incoming.get("data_office"), str): current["data_office"] = incoming["data_office"].strip()[:120]
        date_mode = str(incoming.get("data_date_mode") or "").strip()
        if date_mode in ("current_month", "custom"): current["data_date_mode"] = date_mode
        date_pattern = re.compile(r"^\d{4}-\d{2}-\d{2}$")
        for key in ("data_date_start", "data_date_end"):
            if isinstance(incoming.get(key), str):
                value = incoming[key].strip()
                if value and not date_pattern.match(value): raise ValidationError("Dates must look like YYYY-MM-DD.")
                current[key] = value
        if (current.get("data_date_mode") == "custom" and current.get("data_date_start") and current.get("data_date_end") and current["data_date_start"] > current["data_date_end"]):
        ...
        for key in ("data_date_param_start", "data_date_param_end"):
            if isinstance(incoming.get(key), str): current[key] = incoming[key].strip()[:80]
        ...
        if isinstance(incoming.get("per_team_selected"), str): current["per_team_selected"] = incoming["per_team_selected"][:120]
        ...
        if isinstance(incoming.get("visible_metrics"), dict):
            ...
            for mode in CORE_MODES:
                ...
                if isinstance(values, list): current["visible_metrics"][mode] = [v for v in values if v in valid_keys]
        if isinstance(incoming.get("sort_metric"), dict):
            numeric_keys = {key for key, _, typ in METRIC_DEFS if typ in ("number", "percent", "currency")}
            current.setdefault("sort_metric", {})
            for mode in CORE_MODES:
                value = incoming["sort_metric"].get(mode)
                if value in numeric_keys: current["sort_metric"][mode] = value
        if isinstance(incoming.get("number_font_scale"), dict):
            ...
            for mode in CORE_MODES:
                try:
                    ...
                    if raw is not None: current["number_font_scale"][mode] = min(max(int(raw), 60), 300)
                except Exception: pass
        for key in ("tableau_workbook", "tableau_sheet"):
            if isinstance(incoming.get(key), str): current[key] = incoming[key].strip()[:300]
        if isinstance(incoming.get("source_mapping"), dict):
            raw = incoming["source_mapping"]; metrics = raw.get("metrics") if isinstance(raw.get("metrics"), dict) else {}
            valid = {key for key, _, _ in METRIC_DEFS}
            current["source_mapping"] = {
                "rep_name": str(raw.get("rep_name") or "")[:200], "home_branch": str(raw.get("home_branch") or "")[:200],
                "team": str(raw.get("team") or "")[:200],
                "metrics": {str(key): str(value)[:200] for key, value in metrics.items() if key in valid and str(value or "").strip()},
            }
        if isinstance(incoming.get("source"), dict): current["source"] = clean_source(incoming["source"])
        if isinstance(incoming.get("product_icons"), dict):
            allowed = {"bath", "siding", "windows", "gutters", "roof", "overall"}; icons = {}
            for card, url in incoming["product_icons"].items():
                card = str(card).strip().lower(); url = str(url or "").strip()
                if card in allowed and url.startswith("/api/asset-library/") and ".." not in url: icons[card] = url[:300]
            current["product_icons"] = icons
        try:
            refresh = int(incoming.get("display_refresh_seconds", current.get("display_refresh_seconds", 5)))
            current["display_refresh_seconds"] = min(max(refresh, 2), 60)
        except Exception: pass
        current["rank_direction"] = {mode: "desc" for mode in CORE_MODES}
        self.settings_repo.save(current); self.bump(); return current
