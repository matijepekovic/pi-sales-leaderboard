"""Tableau discovery, preview and manual refresh."""
...
class SourceService:
    def __init__(self, repos, rep_refresh, preview, tableau=None):
        self.repos = repos
    ...
    def options(self):
        ...
        offices = sorted({
            str(rep.get("home_branch") or "").strip()
            for rep in reps if str(rep.get("home_branch") or "").strip()
        })
        names = sorted({
            str(rep.get("rep_name") or "").strip()
            for rep in reps if rep.get("rep_name")
        }, key=str.lower)
    ...
    def test_connection(self):
        ...
        return {
            ...
            "message": (
                f"Connected. {preview['total_rows']} people in {preview['start']} to "
                f"{preview['end']}; {preview['selected_rows']} match your selection."
            ),
        }
    ...
    def refresh(self):
        ...
        try:
            return self.rep_refresh.refresh(self.repos.settings.get())
        finally:
            self._refresh_lock.release()

    def _configured_settings(self):
        runtime = self.tableau.normalized_settings(self.repos.settings.get())
        ...
        if missing:
            raise ValidationError(f"Enter {', '.join(missing)} in Tableau Login first.")
        return runtime
    ...
    def report(self):
        ...
        return {
            ...
            "pat_name": str(config.get("pat_name") or ""),
    ...
    def stop_preview(self):
        return self.preview_store.stop()
