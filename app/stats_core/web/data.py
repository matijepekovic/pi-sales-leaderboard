...
def blueprint(sources, reports):
    ...
    def list_sources():
        try: return jsonify({"ok": True, "sources": sources.list()})
        except Exception as exc: return error_response(exc)

    @bp.post("/api/data/sources")
    def create_source():
        try: return jsonify({"ok": True, "source": sources.save(request.get_json(silent=True) or {})})
        except Exception as exc: return error_response(exc)

    @bp.get("/api/data/sources/<source_id>")
    def get_source(source_id):
        try: return jsonify({"ok": True, "source": sources.get(source_id)})
        except Exception as exc: return error_response(exc)
    ...
    def update_source(source_id):
        ...
        body["id"] = source_id
        try: return jsonify({"ok": True, "source": sources.save(body)})
        except Exception as exc: return error_response(exc)

    @bp.delete("/api/data/sources/<source_id>")
    def delete_source(source_id):
        try: sources.delete(source_id); return jsonify({"ok": True})
        except Exception as exc: return error_response(exc)

    @bp.post("/api/data/sources/<source_id>/test")
    def test_source(source_id):
        try: return jsonify({"ok": True, **sources.test(source_id)})
        except Exception as exc: return error_response(exc)

    @bp.get("/api/data/sources/<source_id>/workbooks")
    def source_workbooks(source_id):
        try: return jsonify({"ok": True, "workbooks": sources.workbooks_for(source_id)})
        except Exception as exc: return error_response(exc)

    @bp.get("/api/data/sources/<source_id>/views")
    def source_views(source_id):
        try: return jsonify({"ok": True, "views": sources.all_views_for(source_id)})
        except Exception as exc: return error_response(exc)

    @bp.get("/api/data/sources/<source_id>/workbooks/<path:workbook>/views")
    def source_workbook_views(source_id, workbook):
        try: return jsonify({"ok": True, "views": sources.views_for(source_id, workbook)})
        except Exception as exc: return error_response(exc)

    @bp.get("/api/data/reports")
    def list_reports():
        try: return jsonify({"ok": True, "reports": reports.list(request.args.get("source_id"))})
        except Exception as exc: return error_response(exc)

    @bp.post("/api/data/reports")
    def create_report():
        try: return jsonify({"ok": True, "report": reports.save(request.get_json(silent=True) or {})})
        except Exception as exc: return error_response(exc)
    ...
    def update_report(report_id):
        body = dict(request.get_json(silent=True) or {})
        body["id"] = report_id
        try: return jsonify({"ok": True, "report": reports.save(body)})
        except Exception as exc: return error_response(exc)

    @bp.delete("/api/data/reports/<report_id>")
    def delete_report(report_id):
        try: reports.delete(report_id); return jsonify({"ok": True})
        except Exception as exc: return error_response(exc)
    ...
    def refresh_report(report_id):
        try: return jsonify({"ok": True, "result": sources.refresh_report(report_id)})
        except Exception as exc: return error_response(exc)

    @bp.post("/api/data/reports/<report_id>/columns")
    def report_columns(report_id):
        try: return jsonify({"ok": True, **sources.columns_for(report_id, request.get_json(silent=True) or {})})
        except Exception as exc: return error_response(exc)

    @bp.post("/api/data/reports/<report_id>/test")
    def test_report(report_id):
        try: return jsonify({"ok": True, **sources.test_report(report_id, request.get_json(silent=True) or {})})
        except Exception as exc: return error_response(exc)

    @bp.post("/api/data/reports/<report_id>/preview")
    def preview_report(report_id):
        try: return jsonify({"ok": True, **sources.preview_report(report_id, request.get_json(silent=True) or {})})
        except Exception as exc: return error_response(exc)
