...
- [x] Production restrictions are centralized behind the production feature-access policy and matching settings guard.
- [x] A central `FEATURE_ACCESS` policy and `can_use(feature)` interface allow all existing Phase 1 features while preserving later per-feature restriction capability.
...
- [x] Windows launcher regression tests passed.
- [x] Windows signed-update tests passed.
...
**Status: ⚠️ IMPLEMENTATION SHIPPED OUT OF ORDER — FIXES IN PROGRESS / NOT CONFIRMED (2026-08-29)**
...
- [x] The refactored runtime is on `production`; current production after the storage cleanup is `95d3f573ecf7bc3b86c7d9a4c349f53a45d5ac12` plus this checklist-only commit.
...
- [x] `windows/test_restructured_runtime.py` now enforces that the retired root-level legacy files must not exist and that plain `import stats_core` loads no Windows-specific module.
- [x] The current runtime/ownership, PIN, persistent-path, repository-storage, launcher, signed-update, packaged-backend, signed-feed, installer, and install/uninstall checks passed on PR #74 / Actions run `33296690333`.
- [x] Windows persistent data has one explicit root under `%LOCALAPPDATA%\Stats\data` with non-destructive one-time migration of the legacy directory.
- [x] Windows Tableau-login/theme-editor helpers receive repositories from the platform layer instead of importing persistence directly.
- [x] The top-level `app/database.py` module is gone.
- [x] `SettingsRepository`, `MetaRepository`, `ProductRepository`, `RepRepository`, and `OrganizationRepository` own their domain SQL directly.
- [x] `stats_core.storage.sqlite` is reduced to constants/schema, connection handling, and install/migration logic rather than acting as a runtime domain facade.
...
5. [ ] Restructured runtime stronger condition — **historical gate result failed** because obsolete files existed on the tested tree. This specific blocker has since been fixed and current CI passes the must-not-exist + platform-neutral import checks; the full one-time parity gate has not been rerun.
...
11. [x] `main` untouched — current repository recheck still finds `main` at `65826125d52d1c249a8072bb78d15108a4c8f9bc`.

## Post-gate fixes completed

- [x] DATA_DIR path + migration: one Windows data root, explicit override, `%LOCALAPPDATA%\Stats\data`, non-destructive legacy copy, and one-time migration semantics are covered by CI and installer tests.
- [x] Retired root-level legacy modules covered by `windows/test_restructured_runtime.py` are physically gone; the test is now “must not exist”.
- [x] Plain `stats_core` import is platform-neutral under the current CI check.
- [x] Windows helper endpoints no longer bypass repositories for Tableau-login/theme-transform persistence.
- [x] Top-level `app/database.py` was removed and persistence was moved under `stats_core/storage`.
- [x] Settings/meta/product persistence ownership moved into their repositories.
- [x] Rep and organization persistence/overlay/team lifecycle ownership moved into their repositories with isolated behavior tests.
- [x] The SQLite module was reduced by removing the dead runtime domain facade; CI prevents those domain functions from returning.
- [x] Full Windows PR build path after the storage cleanup passed through silent install/uninstall.

## Still unresolved before Phase 4 can be confirmed

- [ ] Repair the read-parity verifier's bad baseline catalog path and establish real repository read parity.
- [ ] Isolate the fixed 15-step write replay differences row-by-row/table-by-table and resolve actual behavior differences.
- [ ] Diagnose the complete on-disk theme hash-map difference.
- [ ] Complete automated control server-round-trip/shared-state verification without `/api/config` timing out.
- [ ] Manually verify macro-pad feel on the TV; remote automation cannot prove physical feel.
- [ ] `native_display_mode()` currently returns `(0,0)`, so TV geometry falls back to 1920×1080.
- [ ] Add lockout/rate limiting for `POST /api/auth/unlock`; a 4-digit PIN is brute-forceable at HTTP speed.
- [ ] Remove or deliberately use the dead `STATS_WINDOWS_BUILD` environment flag.
- [ ] Resolve RepTotalsNEW3 empty pull.
- [ ] Verify `Select Start Date` / `Select End Date` parameter names.
- [ ] Resolve stitched-worksheet report behavior.

## 4.1 Storage interfaces

- [x] Add `RepRepository`.
- [x] Add `OrganizationRepository`.
- [x] Add `SettingsRepository`.
- [x] Add `ProductRepository`.
- [x] Add `ThemeRepository`.
- [x] Add asset-library and applied-asset storage interfaces.
- [x] Keep the current database/files and behavior while moving ownership.

## 4.2 Leaderboard service

- [x] Extract leaderboard calculations/business logic into `LeaderboardService`.
- [ ] Core behavior still requires final baseline parity confirmation.

## 4.3 Organization service

- [x] Extract teams, leaders, assignments, team lifecycle, and organization rules into `OrganizationService` / `OrganizationRepository` ownership.

## 4.4 Rep refresh service

- [x] Create one `RepRefreshService`.
- [x] Manual and scheduled refresh use the explicit refresh service path.

## 4.5 Pull policy

- [x] Convert pull policy into an explicit normalization/policy step.
- [x] Remove runtime monkey-patching used to enforce pull policy.

## 4.6 Scheduler

- [x] Reduce scheduler responsibility to scheduling refresh services.
- [x] Remove QR/theme/control/feature installation from scheduler startup.

## 4.7 Data snapshot provider

- [x] Add an explicit data snapshot provider for mapping preview, temporary data, and stored data.
- [x] Remove preview/source monkey-patching from the active runtime.

## 4.8 Screen registry

- [x] Add a central screen registry/interface.
- [x] Register Whole Office.
- [x] Register Per Team.
- [x] Register Team vs Team.
- [x] Register All Teams.
- [x] Register Product Close Rates.

## 4.9 Product service

- [x] Separate Product Source.
- [x] Separate Product Repository.
- [x] Separate Product Refresh Service.
- [x] Separate Product Screen.
- [x] Remove Product monkey-patches from the active server/scheduler/control composition path.

## 4.10 Theme service

- [x] Separate Theme Service.
- [x] Separate Theme Repository.
- [x] Separate Asset Library.
- [x] Separate Applied Asset Store.
- [x] Remove scheduler dependency from active theme handling.

## 4.11 Applied asset protection

- [ ] Full applied-asset/theme parity remains unresolved because the one-time verifier found different complete on-disk hash maps.

## 4.12 Controls

- [x] Add `ControlsService` / central screen-choice ownership.
- [ ] Automated round-trip/shared-state verification remains unresolved.

## 4.13 Authentication

- [x] Move PIN/security behavior into a defined `AuthService`.
- [ ] Add HTTP-speed brute-force protection.

## 4.14 Feature entitlement interface

- [x] Add one central `can_use(feature)`-style interface.
- [x] Keep the development/test entitlement set to all features allowed.
- [x] Do not add payment/account logic yet.

## 4.15 Windows platform layer

- [x] Isolate Windows launching/fullscreen/filesystem/updater/OS-specific operations behind the Windows platform layer.
- [x] Keep core application/business logic platform-neutral under the current import test.

## 4.16 Display frontend cleanup

- [ ] Consolidate the remaining versioned display JS patch stack gradually.
- [x] Establish clear Display, Theme, Layout, Formatting, Controls, and Product runtime ownership.
- [ ] Remove remaining legacy frontend patches only after replacement behavior is verified.

## 4.17 Settings frontend cleanup

- [x] Separate major settings UI code by feature/module.
- [ ] Continue removing legacy/versioned settings patches only after replacement behavior is verified.

## 4.18 Remove obsolete runtime patch architecture

- [x] Remove obsolete root-level runtime feature-install chain from the active server entry.
- [x] Remove obsolete source-preview monkey-patching from the active snapshot path.
- [x] Remove obsolete scheduler composition patching from the active runtime.
- [x] Remove obsolete theme-internal monkey-patching from the active runtime.
...
- [ ] Features no longer modify unrelated feature internals at runtime everywhere, including remaining frontend patch layers.
...
Next work is to resolve the remaining Phase 4 parity failures/unproven checks and carried fix items listed above. Do not begin Phase 5 until Phase 4 passes its exit criteria. `main` must remain untouched.
